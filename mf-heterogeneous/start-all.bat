...
start "Shell" cmd /k "cd mf-shell && npm start"

timeout /t 3

echo Starting Product List (React Remote) - Port 4001...
start "Product List" cmd /k "cd mf-product-list && npm start"

timeout /t 3

echo Starting Product Detail (React Remote) - Port 4002...
start "Product Detail" cmd /k "cd mf-product-detail && npm start"

timeout /t 3

echo Starting Cart (Svelte Remote) - Port 4003...
start "Cart Svelte" cmd /k "cd mf-cart-svelte && npm run dev"
